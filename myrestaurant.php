...
<html>
  ...
  <head>
    <title> Manager Login | DESI KITCHEN </title>
  </head>
...
<?php
$username = $mmail;
$query1 = "SELECT * FROM restaurants WHERE email = '$username'";
	$ses_sql = mysqli_query($conn, $query1);
	$row = mysqli_fetch_assoc($ses_sql);
	$user =$row['name'];
?>
<align = "left">Restaurant Name		:			<?php echo $user;  ?></br>
		Restaurant ID				:			<?php echo $row['R_ID'];  ?></br>
		Restaurant Address			:			<?php echo $row['address'];  ?></br>
		Restaurant contact			:			<?php echo $row['contact'];  ?></br>
		</center>
      </div>
        </form>
